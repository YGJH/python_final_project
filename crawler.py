...
def fetch_page_source(url, port):
    ...
    try:
        ...
        # driver = webdriver.Chrome('C:\\Users\\jihut\\Downloads\\chromedriver_win32\\chromedriver.exe')  # Optional argument, if not specified will search path.
...
def fetch_video_details(link, port):
    ...
    if sub_page_source:
        ...
        # 抓取標籤
        tag_list = []
        tag_container = sub_soup.find("div", class_="tags")
        if tag_container:
            tag_links = tag_container.find_all("a", class_="cat")
            tag_list = [tag.text.strip() for tag in tag_links]
        ...
        for comment_tag in comment_tags:
            ...
            for c in comment_tag:
                comment.append(c.find("p", class_="comment-text").find("span").text)

        video_details = {
            "title": link["title"],
            "url": link_url,
            "models": model_names,
            "tags": " ".join(tag_list),  # 將標籤列表轉換為字串
            "comments": comment
        }
...
def fetch_all_video_details(urls):
    ...
    with open("video_detailss.json", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    'https://jable.tv/models/sakura-koizumi/',
    'https://jable.tv/models/kanae-suzuki/']
